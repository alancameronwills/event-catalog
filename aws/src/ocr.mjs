...
//
// Adapted from server/ocr.js for Lambda: only /tmp is writable, so the
// Tesseract worker and its language data cache live there instead of a data
// dir on disk. /tmp persists across warm invocations of the same execution
// environment but not across cold starts, so language data (a few MB) is
// re-downloaded occasionally rather than shipped in the deployment package —
// simplest option, and fine given how infrequently this runs.
//
// Date/time parsing logic is unchanged from server/ocr.js.
...
async function getWorker() {
  if (!workerPromise) {
    ...
    workerPromise = createWorker("eng", 1, { cachePath: config.ocrCacheDir }).catch((err) => {
